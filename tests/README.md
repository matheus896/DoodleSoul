...
## 6) Scenario 3 (ADK Real API) Evidence Artifacts — Day 5 Closure

For Epic 3 Scenario 3 objective closure, the following artifacts were generated from real ADK sessions:

- `tests/output/scenario3_adk_probe_events_textfallback_long.json`
- `tests/output/scenario3_adk_evidence_summary_textfallback_long.json`

Observed chain for `scene-real-1` in the probe artifact:
...
Frontend acceptance replay summary is generated by:

- `tests/tools/scenario3_frontend_contract_replay.py`
- output: `tests/output/scenario3_frontend_contract_summary_long.json`

Note: backend structured debug lines (`tool_call_recognized`, `scene_orchestration_started`, `media_event_emitted`) may depend on runtime logging configuration. The stream-level chain and frontend replay remain the authoritative functional evidence for this round.
